...
def register(bot, user_data: dict):
    ...
    def pro_screen(message):
        ...
        bot.send_message(
            chat_id,
            "💎 PRO-режим\n\n"
            "PRO даёт более глубокий и точный подбор ухода:\n\n"
            "• больше вариантов на каждом этапе\n"
            "• приоритет проверенных и популярных брендов\n"
            "• расширенный выбор активов\n"
            "• более точный учёт твоих задач\n\n"
            "Можно включить и попробовать 👇",
            parse_mode="Markdown",
            reply_markup=kb
        )
    ...
    def pro_trial(call):
        ...
        bot.send_message(
            chat_id,
            "💎 PRO активирован ✅\n\n"
            "Теперь я буду показывать больше вариантов на каждом шаге "
            "и подбирать уход точнее под твой профиль.\n\n"
            "Можем продолжать 👇",
            reply_markup=main_menu_keyboard()
        )
